...
(use-package
  ...
  ;;          (target-window nil))
  ;;     (cl-loop
  ;;      for win in window-list when
  ;;      (and (window-live-p win)
  ;;           (eq number (string-to-number (window-parameter win 'ace-window-path))))
  ;;      do (setq target-window win) finally return target-window)
  ;;
  ...
  ;;          (interactive)
  ;;          (+aw--select-window ,i))
  ;;        (bind-key (concat "M-" (number-to-string ,i)) #',fn-name))))
  :custom
  (aw-scope 'frame)
  ...
  ;; (cl-loop for i from 1 to 9 do (eval `(+define-avy-switch-to-window ,i)))
  )
...
(use-package
  ...
  ;; :hook (auto-dim-other-buffers-mode . +auto-dim-other-buffers-auto-set-face)
  :custom
  ...
  ;;  :after #'enable-theme
  ;;  (set-face-background
  ;;   'auto-dim-other-buffers-face "#171717"))
  )
...
(use-package
  popper
  :straight t
  :bind (("C-`" . popper-cycle) ("C-~" . popper-toggle-type))
  :hook ((after-init . popper-mode) (after-init . popper-echo-mode))
  :init
  (setq
...
;;  popper
;;  :straight t
;;  :bind (:map popper-mode-map ("C-M-<tab>" . popper-cycle) ("M-`" . popper-toggle-type))
...
;;
;;          chatgpt-shell-mode))
;;  (setq +popper-reference-buffer-no-select '("\\*Warnings\\*"))
;;  (setq popper-reference-buffers
;;        (append +popper-reference-buffers-select +popper-reference-buffer-no-select))
;;  :config
;;  ;; mode-line indicator
...
;;  (defadvice!
;;   +popper-close-window-hack (&rest _)
...
;;
;;  (cl-progv
;;   `(popper-reference-buffers ,@+popper-unpacked-vars)
...
;;    var
;;    in
;;    +popper-unpacked-vars
;;    for
;;    var-no-select
;;    in
;;    +popper-unpacked-vars-no-select
;;    do
;;    (eval `(setq ,var-no-select ',(symbol-value var)))))
;;
...
(use-package zoom :straight t :hook (window-setup . zoom-mode))
